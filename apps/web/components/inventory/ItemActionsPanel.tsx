  ...
  return (
    <Card className="sticky top-24">
      ...
      <CardContent className="space-y-6">
            ...
            {marketplaces.map((market) => (
              <div key={market} className="flex items-center space-x-3">
                <Checkbox
                  ...
                  defaultChecked={item.listedOn.includes(market)}
                />
                <Label htmlFor={market.toLowerCase()} className="font-normal">
                  {market}
                </Label>
              </div>
            ))}
        ...
        <div>
          <p className="text-primary-text mb-2 text-sm font-medium">Status</p>
          <div className="flex flex-wrap gap-2">
            {item.listedOn.length > 0 ? (
              item.listedOn.map((market) => (
                <Badge key={market} variant="primary">
                  {market}
                </Badge>
              ))
            ) : (
